  ...
  override func layoutAttributesForElementsInRect(rect: CGRect) -> [AnyObject]? {
   ...
   let bottomMaxY = collectionView!.contentSize.height + collectionView!.contentInset.bottom - UIScreen.mainScreen().bounds.height
    ...
    if offset.y < topMinY {
      ...
      let deltaY = fabsf(Float(offset.y - topMinY))
      ...
      for a in attrsArray {
        if let attrs = a as? UICollectionViewLayoutAttributes {//从NSMutableArray中取元素，必须unwrap
          if attrs.representedElementKind != nil {//必须判断是否有值
            //            if attrs.representedElementKind == UICollectionElementKindSectionHeader {//nil是不能和String进行相等运算的
            //              var headerRect = attrs.frame
            //              headerRect.size.height = headerSize.height + CGFloat(deltaY)
            //              println("topMinY is \(topMinY), height is \(headerRect.size.height)")
            //              headerRect.origin.y  = headerRect.origin.y - CGFloat(deltaY) + CGFloat(deltaY) * CGFloat(attrs.indexPath.section)
